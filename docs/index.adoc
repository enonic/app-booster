...
Booster application supports `max-age` and `s-maxage` cache directives in response `Cahe-Control` headers. The `max-age` directive is used to specify the maximum amount of time that a response can be cached. The `s-maxage` directive is used to specify the maximum amount of time that a shared cache can cache the response. The `s-maxage` directive is preferred over the `max-age` directive.
Application adds `Age` header to the response. The `Age` header is the time in seconds since the response was cached plus the value of the `Age` header in the response from the upstream.
