...
excludeQueryParams:: Comma-separated list of query parameters that should be excluded from the cache key. Allows to add extra query parameters to exclude from the cache key without need to change default. A token prefixed with `-` removes the given parameter from the preset (e.g. `-cid` drops `cid` from the effective list). By default, no extra query parameters are listed.
...
`excludeQueryParamsPreset` contains a list query parameters that should be excluded from the cache key. The default preset is sourced from https://github.com/mpchadwick/tracking-query-params-registry[mpchadwick/tracking-query-params-registry] plus 5 HubSpot parameters (`_hsenc`, `__hssc`, `__hstc`, `__hsfp`, `hsCtaTracking`) that are not part of that registry. The registry is not included as a dependency, but the preset is maintained by Enonic team and updated periodically.

If you want to keep the default list and add extra query parameters to exclude from the cache key, you can use the `excludeQueryParams` configuration property. Prefixing a parameter with `-` in `excludeQueryParams` removes it from the preset — for example, if your site uses `?cid=` legitimately:

[source,properties]
----
excludeQueryParams = -cid, my_custom_param
----

To override the presets entirely, simply add your own version of `excludeQueryParamsPreset` in the configuration file. The `-` prefix is only honored in `excludeQueryParams`, not in `excludeQueryParamsPreset`.
